...
def parse(csv_string)
  ...
  rows.shift(2); rows.pop(2) # 行頭行末の2行は不要なので捨てる

  rows.each_with_object(Hash.new { |h, k| h[k] = {} }) do |row, holiday_map|
    row.each_slice(2).each do |name, date_string|
      date = Date.parse(date_string)
      holiday_map[date.year][date] = name
    end
  end
end
